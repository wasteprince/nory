...
![Интерфейс NORY 0.2.26: подключение, статистика и карточки серверов](docs/previews/home.png)

## Новое в 0.2.26

Чёрно-серое оформление, новый логотип и компактные карточки серверов вместо длинного списка.
Поиск серверов убран; названия, подписи, протоколы и пинг сохранены. Главная страница
прокручивается целиком, нижняя навигация не закрывает содержимое. Настройки разделены
на вкладки, а в обходе появился встроенный переключатель «Россия — напрямую» для обоих ядер.
...
| --- | --- | --- |
| Arch Linux, x86_64 | `nory-…-x86_64.pkg.tar.zst` | `sudo pacman -U ./nory-0.2.26-1-x86_64.pkg.tar.zst` |
| Debian-подобные, amd64 | `nory_…_amd64.deb` | `sudo apt install ./nory_0.2.26_amd64.deb` |
