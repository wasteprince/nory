...
![Интерфейс NORY 0.2.27: подключение, подписка и карточки серверов](docs/previews/home.png)

## Новое в 0.2.27

Доработанное чёрно-серое оформление: мягкое освещение, плавные переходы и карта мира
по береговым линиям Natural Earth. Карта центрируется и полностью помещается в окно.
Тонкие полосы прокрутки не перекрывают элементы и скрываются после остановки.
Название подписки, статистика и действия объединены в одну панель; длинные названия
серверов помещаются в две строки. В настройках обновлены вкладки и размеры полей.
...
| --- | --- | --- |
| Arch Linux, x86_64 | `nory-…-x86_64.pkg.tar.zst` | `sudo pacman -U ./nory-0.2.27-1-x86_64.pkg.tar.zst` |
| Debian-подобные, amd64 | `nory_…_amd64.deb` | `sudo apt install ./nory_0.2.27_amd64.deb` |
