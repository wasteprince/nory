...
Возвращены цвета тёмной темы из 0.3.9: серый фон, фиолетовые акценты и мягкий
зелёный цвет подключения. Светлая палитра сохранена. Переключение темы доступно
в «Настройки → Приложение»; кнопка в заголовке окна убрана.

Таймер и трафик обновляются в отдельных компонентах, не запуская повторный обход
списка серверов. Данные подписок не оборачиваются в глубокие реактивные объекты,
форматирование чисел и времени использует общие форматтеры. Элементы списков
за пределами видимой области пропускают отрисовку в поддерживающих WebView.

Пустые и неудачные результаты пинга больше не показываются как `n/a`.
Автоматический пинг в режиме «Прокси · HTTPS» отключён; ручная проверка доступна.

При запуске выполняется фоновая проверка подписанного манифеста GitHub Releases.
Если найдена новая версия, окно NORY показывается с диалогом обновления, описанием
и кнопками «Позже» и «Скачать и установить». Диалог ждёт завершения другого
открытого диалога. Ошибки фоновой проверки не мешают запуску.

Изменения 0.3.10 сохранены:
...
Добавлены светлая и тёмная темы с сохранением выбора.
